...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return questions.stream().filter(e -> e.getCategory().equals(category)).toList().size();
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().flatMap(e -> e.getAllAnswers().stream()).anyMatch(e -> e.equals(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return questions.stream().flatMap(e -> e.getAllAnswers().stream()).filter(e -> e.equals(answerCandidate)).toList().size();
